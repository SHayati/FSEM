...
align_sign <- function(est, tru) {
  ...
  for (col in seq_len(d[2])) {
    if (mean(est[, col] * tru[, col]) < 0) est[, col] <- -est[, col]
  }
  est
}
  ...
  for (nm in ind) {
    ...
    if (!is.null(ef) && !is.null(tf)) {
      for (k in intersect(names(ef), names(tf))) {
        if (is.matrix(ef[[k]]) || is.matrix(tf[[k]])) {
          out$lambda <- c(out$lambda, l2_mat(ef[[k]], tf[[k]]))
        } else {
          out$lambda <- c(out$lambda, l2_vec(ef[[k]], tf[[k]]))
        }
      }
    }
    ...
    if (!is.null(est_eval$sigma.fac.eigvec.std[[nm]]) && !is.null(sim$params.org.eval$sigma.fac.eigvec.std[[nm]])) {
      ...
      for (k in seq_len(kmax)) {
        out$phi <- c(out$phi, l2_vec(ev[, k], sim$params.org.eval$sigma.fac.eigvec.std[[nm]][, k]))
      }
    }
  ...
  for (nm in lat) {
    ...
    if (!is.null(es) && !is.null(ts)) {
      for (k in intersect(names(es), names(ts))) {
        if (is.matrix(es[[k]]) || is.matrix(ts[[k]])) {
          out$gamma <- c(out$gamma, l2_mat(es[[k]], ts[[k]]))
        } else {
          out$gamma <- c(out$gamma, l2_vec(es[[k]], ts[[k]]))
        }
      }
    }
    ...
    if (!is.null(est_eval$sigma.sem.eigvec.std[[nm]]) && !is.null(sim$params.org.eval$sigma.sem.eigvec.std[[nm]])) {
      ...
      for (k in seq_len(kmax)) {
        out$psi <- c(out$psi, l2_vec(ev[, k], sim$params.org.eval$sigma.sem.eigvec.std[[nm]][, k]))
      }
    }
...
combo <- sub(".*_N_([0-9]+)_M_([0-9]+)\\.rds", "N=\\1,M=\\2", basename(files))
agg <- list()
...
for (i in seq_along(files)) {
  ...
  if (is.null(sc)) next
  key <- combo[i]
  if (is.null(agg[[key]])) agg[[key]] <- list()
  agg[[key]][[length(agg[[key]]) + 1]] <- sc
}

cat("SIM3 single-run standardized MSE means by group\n")
print(rbind(sim3_well = well, sim3_miss = miss))

cat("\nSIM2 Monte-Carlo mean standardized MSE by (N,M)\n")
for (k in sort(names(agg))) {
  mat <- do.call(rbind, agg[[k]])
  cat("\n", k, " replicates=", nrow(mat), "\n", sep = "")
  print(colMeans(mat, na.rm = TRUE))
}

if (!is.null(agg[["N=50,M=20"]])) {
  ref <- colMeans(do.call(rbind, agg[["N=50,M=20"]]), na.rm = TRUE)
  cat("\nSide-by-side vs N=50,M=20 reference\n")
  print(rbind(sim3_well = well, sim3_miss = miss, sim2_ref = ref))
}
